...
import { Sword, Shield, Crown, Flame, BookOpen } from "lucide-react"
...
export const NarasimhaCard: React.FC<NarasimhaCardProps> = ({
  ...
  const labels = {
    en: { meet: "Character", place: "Location", fact: "Divine Fact", readMore: "Read Divine Details", loading: "Fetching Puranas...", close: "Close" },
    hi: { meet: "पात्र", place: "स्थान", fact: "दिव्य तथ्य", readMore: "विस्तृत विवरण पढ़ें", loading: "खोज रहे हैं...", close: "बंद करें" },
    or: { meet: "ପାତ୍ର", place: "ସ୍ଥାନ", fact: "ଦିବ୍ୟ ତଥ୍ୟ", readMore: "ବିସ୍ତୃତ ବିବରଣୀ ପଢନ୍ତୁ", loading: "ଖୋଜୁଛି...", close: "ବନ୍ଦ କରନ୍ତୁ" }
  }
  ...
  const fetchDetailedContent = async () => {
    ...
    setIsLoading(true)
    try {
      const response = await fetch(`/api/narasimha-story-detail?storyId=${id}`);
      if (!response.ok) {
      ...
      <div className="w-full md:w-1/2 flex flex-col space-y-6">
        ...
        <div className="flex justify-start mt-6">
          <button
            ...
            disabled={isLoading}
            className="group/btn relative px-6 py-3 rounded-full bg-gradient-to-r from-amber-600 to-orange-700 text-white font-bold text-sm uppercase tracking-widest shadow-lg hover:shadow-orange-500/40 transition-all active:scale-95 disabled:opacity-50 disabled:cursor-wait overflow-hidden"
          >
